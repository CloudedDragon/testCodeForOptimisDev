...
import net.eocbox.volleyeocbox_lib.R;


/**
 * Created by Bruce_Lan on 15/9/11.
 */
public class CircleNetworkImageView extends CustomNetworkImageView {

    // custom for circle image
    ...
    private void setup() {
//        Log.d("CustomCircleNetworkImageView", "setup()");

        if (!mReady) {
//            Log.d("CustomCircleNetworkImageView", "if (!mReady)");
            mSetupPending = true;
            return;
        }

        if (mBitmap == null) {
//            Log.d("CustomCircleNetworkImageView", "if (mBitmap == null)");
            return;
        }
